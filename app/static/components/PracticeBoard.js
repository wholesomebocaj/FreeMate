    ...
    play(type) {
        try {
            ...
            const [frequency, duration, wave] =
                this.tones[type] || this.tones.move;
...
export class PracticeBoard {
    ...
    installSquareLayer() {
        ...
        for (let rank = 8; rank >= 1; rank -= 1) {
            files.forEach((file) => {
                ...
                square.className =
                    (rank + files.indexOf(file)) % 2 === 0 ? "light" : "dark";
    ...
    setConfig(config = {}) {
        ...
        this.lockToAllowedMoves =
            config.lockToAllowedMoves ?? this.lockToAllowedMoves;
        this.highlightLegalMoves =
            config.highlightLegalMoves ?? this.highlightLegalMoves;
        this.onMoveSuccess = config.onMoveSuccess || this.onMoveSuccess;
        this.onMoveError = config.onMoveError || this.onMoveError;
    ...
    async handleSelect(square) {
        ...
        this.ground.set({
            movable: {
                dests: legalSquares.length
                    ? new Map([[square, legalSquares]])
                    : new Map(),
            },
        });
    }
    ...
    async tryMove(fromSquare, toSquare, metadata = {}) {
        ...
        const captured =
            Boolean(this.position[toSquare]) || Boolean(metadata.captured);
    ...
    isConfiguredMoveAllowed(move) {
        ...
        if (this.lockToAllowedMoves && this.allowedMoves.length) {
            return (
                this.allowedMoves.includes(move) ||
                this.allowedMoves.includes(baseMove)
            );
        }
        if (this.mode === "puzzle" && this.solutionMoves.length) {
            return (
                this.solutionMoves[0] === move ||
                this.solutionMoves[0] === baseMove
            );
        }
        return true;
    }
    ...
    rejectMove(move, message) {
        ...
        this.syncBoard({ clearSelection: true });
        const errorData = { move, message };
        this.onIllegalMove(errorData);
        if (this.mode === "lesson" || this.mode === "puzzle") {
            this.onMoveError(errorData);
        }
    }
